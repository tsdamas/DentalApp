...
if (empty($_SESSION['username'])) {
    ...
    $get_proc_sql = "SELECT a.id AS id, CONCAT(p.fname, ' ', p.lname) AS patient_name, d.name AS dentist_name, roomNo" . " FROM L4_Appointments a, L4_Dentists d, L4_Patients p WHERE a.pat_id = p.id and a.den_id = d.id and app_date = '2024-12-05'";
    ...
    if (mysqli_num_rows($get_proc_query) > 0) {
        ...
        while ($info = mysqli_fetch_array($get_proc_query)) {
            $row .= '<tr>' . '<td>' . $info['id'] . '</td>' . '<td>' . $info['patient_name'] . '</td>' . '<td>' . $info['dentist_name'] . '</td>' . '<td>' . $info['roomNo'] . '</td></tr>';
        }
    }
}
?>
...
<body class="bg-body-tertiary">
	...
	<div class="container">
		...
		<div class="row">
			<div
				class="col-lg-6 col-xs-12 rounded shadow-sm p-3 mt-5 mb-5 bg-white"
				id="app-summary">
